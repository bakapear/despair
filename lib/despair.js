 ...
 * Flattens an array.
 */
Despair.prototype.flat = function (arr, depth) {
  depth = depth || 500
  let save = []
  for (let i = 0; i < depth; i++) {
    arr = [].concat(...arr)
    if (JSON.stringify(arr) === JSON.stringify(save)) break
    save = arr.slice()
  }
  return arr
}
...
/**
 * Uses a hash function on a string. Returns null if unsupported type.
 */
Despair.prototype.hash = function (str, type) {
  type = type.toLowerCase()
  let hashes = crypto.getHashes().map(x => x.toLowerCase())
